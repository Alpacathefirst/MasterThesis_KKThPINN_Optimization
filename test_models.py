...
# Load ONNX model
session = ort.InferenceSession(r'C:\Users\caspe\PycharmProjects\KKThPINN_Optimization\models\flash\ECNN\0.2\test2_0.2_0.onnx')

df = pd.read_csv(r'C:\Users\caspe\PycharmProjects\KKThPINN_Optimization\data_generation\data_files\VLE_H_dataset_reordered_2')
...
NN_outputs = ["CO2(g).1", "H2O(g)", "N2(g).1", "HCO3-", "Na+", "CO2(aq)", "H2O(aq).1", "N2(aq)", "CO3-2", "OH-", "H+", "NaOH(aq).1", "enthalpy"]
...
for _, row in df.iterrows():
    ...
    X_scaled = X / scaler.scale_[:6]  # shape must match training input dim
