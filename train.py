...
def run_training(args, data):
    ...
    if args.job == 'experiment':
        scores = evaluate_model(data, args, device)
...
def evaluate_model(data, args, device):
    rmse_total = 0
    ...
    if args.model == 'NN':
        ...
        # device = data['B'].device  # or use pred.device or chunk.device later
        B = data['B'].to(device)
        A = data['A'].to(device)
        b = data['b'].to(device)

        chunk = torch.mm(B.t(), torch.inverse(torch.mm(B, B.t())))
        Astar = -torch.mm(chunk, A)
        Bstar = torch.eye(args.z0_dim, device=chunk.device) - torch.mm(chunk, B)
        bstar = torch.matmul(chunk, b).squeeze(-1)
